...
def call_gpt(user_prompt, system_prompt = COMMON_SYSTEM_PROMPT, model = None, temperature = 0.0, num_completions = 1, regenerte_answer = False, pre_msgs = None):
    ...
    for attempt in range(10):
        try:
            ...
            responses = type('obj', (object,), {
                'choices': [MockResponse(response_text) for _ in range(num_completions)],
                'usage': {'model': model}
            })
            break
            ...
            if attempt > 5:
                full_prompt = full_prompt[:int(len(full_prompt) * 0.6)]
            print(f"Retrying... attempt {attempt + 1}/10")
            import time
            time.sleep(15)
            continue
